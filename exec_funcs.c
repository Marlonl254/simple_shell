...
/**
 * execute_fork - Create a process to execute a command
 * @c_path: command to execute
 * @args: The arguments to be executed
 * @av: array of the command line arguments given when the program is run
 * @ev: Environment variables
 * @cmd_cnt: command number
 * @status: Pointer at the exit status of the last executed command
 * @commands: commands given to be set free
 *
 * Return: void
 */
...
/**
 * exec_line_commands - Execute commands gotten from stdin
		    ...
		    int *status)
{
	...
	while (commands && commands[m])
	{
		char **args = _strtok(commands[m], " \t");
		...
		{
			free_arr(commands);
			exit_program(args, m, cmd_cnt, status);
		}
		if (_strchr(args[0], '/') == NULL)
		{
			...
			c_path = args[0];
		if (is_path_null(c_path, status, &m, args, av, cmd_cnt))
			continue;
 ...
 * appropriate action if true
 * @c_path: extracted command path
 * @status: Pointer to the status of last executed command
